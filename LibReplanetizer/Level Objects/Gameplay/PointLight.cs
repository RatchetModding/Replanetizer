...
{
    public class PointLight : LevelObject, IRenderable
    {
        ...
        public PointLight(byte[] block, int num)
        {
            ...
            x = ReadShort(block, offset + 0x00) * (1.0f / 64.0f);
            y = ReadShort(block, offset + 0x02) * (1.0f / 64.0f);
            z = ReadShort(block, offset + 0x04) * (1.0f / 64.0f);
            radius = ReadShort(block, offset + 0x06) * (1.0f / 64.0f);
            red = ReadUshort(block, offset + 0x08);
            green = ReadUshort(block, offset + 0x0A);
            blue = ReadUshort(block, offset + 0x0C);
        ...
        public override byte[] ToByteArray()
        {
            ...
            WriteShort(bytes, 0x00, (short) MathF.Round(x * 64.0f));
            WriteShort(bytes, 0x02, (short) MathF.Round(y * 64.0f));
            WriteShort(bytes, 0x04, (short) MathF.Round(z * 64.0f));
            WriteShort(bytes, 0x06, (short) MathF.Round(radius * 64.0f));
            WriteUshort(bytes, 0x08, red);
            WriteUshort(bytes, 0x0A, green);
            WriteUshort(bytes, 0x0C, blue);
