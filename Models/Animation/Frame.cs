...
namespace RatchetEdit.Models.Animations
{
    public class Frame
    {
        ...
        public byte[] Serialize()
        {
            ...
            {
                WriteShort(ref rotationBytes, i * 8 + 0x00, rotations[i][0]);
                WriteShort(ref rotationBytes, i * 8 + 0x02, rotations[i][1]);
                WriteShort(ref rotationBytes, i * 8 + 0x04, rotations[i][2]);
                WriteShort(ref rotationBytes, i * 8 + 0x06, rotations[i][3]);
            }
            byte[] sec0Bytes = new byte[sec0s.Count * 8];
            for (int i = 0; i < sec0s.Count; i++)
            {
                WriteShort(ref sec0Bytes, i * 8 + 0x00, sec0s[i][0]);
                WriteShort(ref sec0Bytes, i * 8 + 0x02, sec0s[i][1]);
                WriteShort(ref sec0Bytes, i * 8 + 0x04, sec0s[i][2]);
                WriteShort(ref sec0Bytes, i * 8 + 0x06, sec0s[i][3]);
            }
            byte[] translationBytes = new byte[translations.Count * 8];
            for (int i = 0; i < translations.Count; i++)
            {
                WriteShort(ref translationBytes, i * 8 + 0x00, translations[i][0]);
                WriteShort(ref translationBytes, i * 8 + 0x02, translations[i][1]);
                WriteShort(ref translationBytes, i * 8 + 0x04, translations[i][2]);
                WriteShort(ref translationBytes, i * 8 + 0x06, translations[i][3]);
            }
            ...
            WriteFloat(ref header, 0x00, speed);
            WriteUshort(ref header, 0x04, frameIndex);
            WriteUshort(ref header, 0x06, frameLength);
            WriteUshort(ref header, 0x08, sec0Pointer);
            WriteUshort(ref header, 0x0A, (ushort)sec0s.Count);
            WriteUshort(ref header, 0x0C, translationPointer);
            WriteUshort(ref header, 0x0E, (ushort)translations.Count);
